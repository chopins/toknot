...
const zend_function_entry epoll_functions[] = {
	PHP_FE(confirm_epoll_compiled,	NULL)		/* For testing, remove later. */
	PHP_FE_END	/* Must be the last line in epoll_functions[] */
};
/* }}} */
...
/* {{{ proto string confirm_epoll_compiled(string arg)
   Return a string to confirm that the module is compiled in */
PHP_FUNCTION(confirm_epoll_compiled)
{
	char *arg = NULL;
	int arg_len, len;
	char *strg;
	...
	len = spprintf(&strg, 0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "epoll", arg);

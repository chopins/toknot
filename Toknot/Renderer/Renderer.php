...
class Renderer extends Object {
    ...
    public static function helper($CFG) {
        self::$cachePath = FileObject::getRealPath($this->appRoot, $CFG->templateCompileFileSavePath);
        self::$fileExtension = $CFG->templateFileExtensionName;
        self::$scanPath = FileObject::getRealPath($this->appRoot, $CFG->templateFileScanPath);
        self::$htmlCachePath = FileObject::getRealPath($this->appRoot, $CFG->htmlStaticCachePath);
        self::$outCacheThreshold = $CFG->defaultPrintCacheThreshold;
        self::$dataCachePath = $CFG->dataCachePath;
     ...
     * 
     * @param array|Toknot\Boot\ArrayObject $vars
     */
    ...
    public function display($tplName) {
        ...
        if (!file_exists($tplFile)) {
            throw new BaseException("{$tplFile} not exists");
        }
        $useCache = false;
        //HTML cache control

...
class Renderer extends Object {
    ...
    public function convertTemplateToPHP($templateFile) {
        $rfp = fopen($templateFile, 'r');
        $wfp = fopen($convertFile, 'w');
        while (!feof($fp)) {
    ...
    /**
     * parse template of variable to php 
     *
     * @param mixed $str
     * @access private
     * @return void
     ...
     * @return void
     */
    public function displays($obc = true) {
    ...
    public function fetch_templete() {
        ...
        if (__X_SHOW_ERROR__ || $comp_file_time <= $guess_path_time) {
            ...
            if ($this->T->type == 'json') {
                ...
                $this->parseHtmlTpl($tpl_file, $cache_file);
            }
        }
    }
     ...
     * @return void
     */
    private function parseHtmlTpl($tpl_file, $comp_file) {
        $file_str = file_get_contents($tpl_file);
        $this->parseVar($file_str);
        $this->echoValue($file_str);
        $this->callFunc($file_str);
        $this->parseInc($file_str);
        $this->parseJs($file_str);
        $this->parseCss($file_str);
        $this->parseIf($file_str);
        $this->parseForeach($file_str);
        $this->parseSet($file_str);
        $this->parseUri($file_str);
        if ($this->TPL_INI->compression) {
            $this->delHtmlComment($file_str);
     ...
     * @return void
     */
    private function parseForeach(&$str) {
     ...
     * @return void
     */
    private function delHtmlComment(&$str) {
     ...
     * @return void
     */
    private function incJs($file) {
        $file_path = "{$_ENV['__X_APP_UI_DIR__']}/{$this->TPL_INI->js_file_dir}/{$file}.js";
        if (!file_exists($file_path))
            throw new XException("{$file_path} not exists");
     ...
     * @return void
     */
    private function echoValue(&$str) {
     ...
     * @return void
     */
    private function callFunc(&$str) {
        $str = preg_replace('/\{func\s+([a-zA-Z_\d]+)\((.*)\)\}/i', $this->tnd . 'if(function_exists(\'$1\')){ $this->out_html.=$1($2);} elseif(method_exists($this,\'$1\')) {

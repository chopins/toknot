if (typeof TK == 'undefined') {
    ...
    var TK = {
        doc: window.document,
        xPNode: window.document.body, //TK.$(ele)方法返回对象的父对象
        isReady: false,
        ...
        init: function() {
            window.onload = function() {
                ...
                if (TK.doc) {
                    TK.doc.onkeydown = TK.kEF;
                    TK.doc.onkeyup = TK.kEF;
                    TK.doc.onmousedown = TK.bMECF;
                    TK.doc.onmouseup = TK.bMECF;
                    TK.doc.onmouseover = TK.mMEF;
                    TK.doc.onmouseout = TK.mMEF;
                    window.onscroll = TK.wSF;
                    if (window.top == window.self) {
                        TK.doc.body.onresize = TK.wRSF;
                    }
                }
        ...
        wSF: function(e) {
            e = e || event;
            for (var i in TK.wSFL)
                if (!isNaN(i))
                    TK.wSFL[i].call(TK.wSFL[i].handObj, e);
        },
        //窗口改变大小事件
        wRSF: function(e) {
            for (var i in TK.wRSFL)
                if (!isNaN(i))
                    TK.wRSFL[i].func(TK.wRSFL[i].obj);
        },
        //添加鼠标移动事件函数
        aMMECF: function(func, type) {
            TK.bMECFL[type].push(func);
        },
        //添加document鼠标点击事件
        aBMECF: function(func, type, button) {
            ...
            if (!button)
                button = 3;
            TK.bMECFL[type][button].push(func);
        },
        //document鼠标移动事件回调函数
        mMEF: function(e) {
            e = e || event;
        ...
        //document鼠标点击事件回调函数
        bMECF: function(e) {
            e = e || event, fL = TK.bMECFL[e.type][e.button], fL = fL.concat(TK.bMECFL[e.type][3]);
            for (var i in fL)
                !isNaN(i) && fL[i](e);
        },
        //document键盘事件回调函数
        ...
        dMEP: function(type) {
            return {left: function(func) {
                    TK.aBMECF(func, type, 0);
                },
                right: function(func) {
                    TK.aBMECF(func, type, 2);
                },
                middle: function(func) {
                    TK.aBMECF(func, type, 1);
                },
                any: function(func) {
                    TK.aBMECF(func, type, 3);
                }
            };
        },
        mouseover: function(func, eventObj) {
            func.eventObj = eventObj;
            TK.aMMECF(func, 'mouseover');
        },
        mouseout: function(func, eventObj) {
            TK.aMMECF(func, 'mouseout', eventObj);
        },
        mousedown: function() {
            return TK.dMEP('mousedown');
        },
        mouseup: function() {
            return TK.dMEP('mouseup');
        },
                    ...
                    switch (firstWord) {
                        case '#': //样式名
                            return (function(clsName) {
                                var list = Array();
                                var childList = TK.$(this.xPNode).getChilds();
